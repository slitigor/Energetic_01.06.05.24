...
public class AddressValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (repository.findByZip(address.getZip()).isPresent())
            errors.rejectValue("zip", String.format(
                    "The address with the specified zip code '%s' already exists in the database.",
                    address.getZip()));
        if (repository.findByCityAndStreet(address.getCity(), address.getStreet()).isPresent())
            errors.rejectValue("city", String.format("City and Street: '%s, %s' already exists in the database",
